...
var countDownDate = new Date("March 13, 2024 16:00:00").getTime();
...
1. Öffne dein `Datascience mit R` Projekt. Es sollte die Ordner `1_Data` und `2_Code` enthalten. Stelle sicher, dass du alle Datensätze, welche im `Datensätze` Tab aufgelisted sind, in deinem `1_Data` Ordner hast. 

2. Öffne ein neues R Skript. Schreibe deinen Namen, das Datum und "Wettbewerb" als Kommentare an den Anfang des Skripts.

3. Speichere das neue Skript unter dem Namen `Name_Wettbewerb.R` im `2_Code` Ordner.
...
4. Du hast einen Telefonjoker: Du kannst mich **einmal** konsultieren und um Hilfe bitten.

5. Vor dem Einreichen, speichere deine Graphik mittels `ggsave()` (siehe code) und benennt die Datei mit deinem *Namen*. Benutzt deinen *Namen* ebenfalls im Namen der `.R` Skriptdatei (siehe oben). 

```{r}
# speichere plot
ggsave(filename = "NAME_plot.png",
       plot = mein_plot,
       device = "png",       
       units = 'in',
...
- [**Graphik + Skript einreichen**](https://www.dropbox.com/request/H8dN3B8vm81zifcTjovP)

7. Jeder Beitrag wird durch eine Jury beurteilt, diese besteht aus den KursteilnehmerInnen. Der Beitrag mit dem höchsten Punktemittel gewinnt.   
...
Bitte rated die einzelnen Beiträge über diesen [**Link**](https://formr.imucb.ch/RDSCompetition)
